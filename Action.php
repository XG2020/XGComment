...
class XGComment_Action extends Widget_Abstract_Users implements Widget_Interface_Do
{
    /**
     * 插件配置
     *
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        // 获取插件配置信息
        $this->_plugin = $this->options->plugin('XGComment');
    }
    ...
    {
        // 创建表单
        $form = new Typecho_Widget_Helper_Form($this->security->getIndex('action/password'), Typecho_Widget_Helper_Form::POST_METHOD);
        // 邮箱地址
    ...
    private function doForget()
    {
        ...
        // 生成链接地址
        $uri = Typecho_Common::url('/action/password?' . http_build_query($query), $this->options->index);
        // 请求参数
        $data = array(
        ...
        switch ( $this->_plugin->public_interface ) {
            case 'sendcloud': // Send Cloud
                ...
                // 是否成功
                if ( !XGComment_Plugin::sendCloud($data) ) {
                    // 输出错误信息
            ...
            case 'aliyun': // 阿里云
                ...
                // 是否成功
                if ( !XGComment_Plugin::aliyun($data) ) {
                    // 输出错误信息
                    $this->widget('Widget_Notice')->set(_t('邮件发送失败, 请联系管理员解决!'), 'error');
                    // 跳转回去
                    $this->response->goBack();
    ...
    private function reset()
    {
        ...
        if ( time() > $expire ) {
            ...
            // 跳转到找回密码界面
            $this->response->redirect(Typecho_Common::url('/action/password?forget', $this->options->index));
        }
        // 查询用户数据
