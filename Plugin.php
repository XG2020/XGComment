...
class XGComment_Plugin implements Typecho_Plugin_Interface
{
    /**
     * 激活插件
     *
    ...
    static public function activate()
    {
        ...
        // 找回密码动作添加
        Helper::addAction('password', 'XGComment_Action');
        // 找回密码路由添加
        Helper::addRoute('password_forget', '/password/forget', 'XGComment_Action', 'forget');
        Helper::addRoute('password_reset', '/password/reset', 'XGComment_Action', 'reset');
    }
    ...
    {
        // 删除动作
        Helper::removeAction('password');
        // 删除路由
        Helper::removeRoute('password_forget');
    ...
    static public function configCheck(array $settings)
    {
        ...
        // 获取插件配置
        $plugin = $options->plugin('XGComment');
        // 是否启用配置验证
    ...
    static public function sendCloud($data)
    {
        ...
        $flag = TRUE;
        // 获取插件配置
        $plugin = Helper::options()->plugin('XGComment');
        // 如果开启了Debug
        ...
        if ( in_array('enable', $plugin->public_forget) ) {
            ...
            if ( preg_match('/\/login\.php/i', $pathinfo) ) {
                ...
                <script>
                    ...
                    forget.href = '<?php echo Typecho_Common::url('/action/password?forget', Helper::options()->index);?>';
